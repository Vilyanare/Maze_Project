...
# tell sdl2 where your library is.  Uncomment and add filename.
# Must run from root directory of project and have library on same level
'''
library_name = ""
current_dir = os.getcwd()
os.environ["PYSDL2_DLL_PATH"] = current_dir + library_name
'''
# import sdl2
import sdl2
# and sdl2.ext, where the pythonic part of the pysdl2 resides
import sdl2.ext
# graphics module and text render mondule for SDL2
import sdl2.sdlgfx, sdl2.sdlttf
...
from constants import (SCREEN_WIDTH, SCREEN_HEIGHT, TILE_SIZE, LIMIT_FPS,
                       WINDOW_COLOR, TITLE, RAY_ANGLE, CENTER_Y,
                       TILE_SIZE, VIEW_PANE_DISTANCE, BORDER)
...
class Manager(object):
    ...
    def __init__(
        ...
        limit_fps=None, window_color=None, border=BORDER, title=None
    ):
        """Initialization.
        ...
        self.window_color = window_color if window_color else WINDOW_COLOR
        self.title = title or TITLE
        self.border = None if not border else sdl2.SDL_WINDOW_BORDERLESS
        ...
        self.window = sdl2.ext.Window(
            self.title, size=(self.width, self.height),
            flags=self.border)
    ...
    def polygonRGBA(self, x, y , n, color):
        '''
            wrapper for SDL2GFX method polygonRGBA
        '''
        x = (ctypes.c_int16 * len(x))(*x)
        y = (ctypes.c_int16 * len(y))(*y)

        sdl2.sdlgfx.filledPolygonRGBA(self.renderer.sdlrenderer, x, y, n, *color)
...
if __name__ == '__main__':
    ...
    m = Manager(window_color=(34, 124, 214, 255))
